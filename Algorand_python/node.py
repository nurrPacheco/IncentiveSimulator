...
class Node(object):
	...
	def BAstartCommitteVote(self,ev,roundNumber,stepNumber,touCommitte,flag,\
		...
		if resp.j > 0:  # If I have own the committe
			print(BOLDMAGENTA("=>"),self.nodeId,"sortition-bastar-step",str(ev.stepNumber),\
				"round",str(ev.roundNumber),RESET("")) ##Active Event (DATABASE?)
			prevBlock = self.blockChain[len(self.blockChain) - 1]
			prevBlockHash = H(prevBlock)
	...
	def BAstartCountVoteOne(self,ev):
		print(self.nodeId, " BA* Count Vote ONE is executing in step = "\
			,ev.stepNumber) #Active Event (DATABASE?)
		step = ev.stepNumber
	...
	def finalCountVote(self,ev):
		conn = pymysql.connect("localhost", "root", "AcydLab+123", "User_Information")
		c = conn.cursor()
		...
		if r == self.bastarOutput:
			...
			print(BOLDGREEN("[FINAL"),RESET(""),str(self.nodeId),"](new block added)=", \
					...
					,"at time" , ev.evTime)
			finalBlocks = H(self.bastarBlock)
		
			algo_deposit = random.randint(1000, 10000)
			MEU = 1
			num_blocks = 0
			...
			for i in range(len(allNodes)):
				...
				if user_stake >= MEU:
					UEU = user_stake / MEU
					fixed_per_block = algo_deposit / num_blocks
					for j in range(num_blocks):
						TEU += UEU
						user_reward = (UEU / TEU) * fixed_per_block
					print("The total amount of rewards for node " + str(i+1) + " is " + str(round(user_reward, 2)))
					c.execute("INSERT INTO Algorand_Simulation (Round, Time, Node, Rewards) VALUES (%s,%s,%s,%s)", (ev.roundNumber, ev.evTime, i+1, round(user_reward, 2)))
					conn.commit()
				else:
					print("The user " + str(self.nodId) + " will not receive any rewards.")
